...
function main() {
    const model = new (0, _model.Model)(ACTIVITY_URL);
    const view = new (0, _view.View)();
    document.addEventListener("requestActivityEvent", (event)=>{
...
},{"./model":"1hsjm","./view":"1ce4O"}],"1hsjm":[function(require,module,exports) {
 ...
 */ async function typedFetch(url, options) {
    ...
    if (contentLength && contentLength !== "0") // Type of unmarshaled response needs to be validated
    return response.json();
    else // No content
    throw new Error(`unexpected empty response`);
}
class Model {
    constructor(boredUrl){
        this.url = boredUrl;
    }
    /**
   ...
   */ getActivity(types) {
        if (types.length === 0) return typedFetch(`${this.url}`);
        const type = types[Math.floor(Math.random() * types.length)];
        return typedFetch(`${this.url}?type=${type}`);
    }
}
...
},{}],"1ce4O":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "View", ()=>View);
/* A web component to gather user input */ class InputForm extends HTMLElement {
    constructor(){
        super();
      ...
      `;
        else throw new Error("shadoRoot does not exist");
    }
    // every time rendered
    connectedCallback() {
    ...
    handleSubmit(eveent) {
        ...
        for(let idx = 0; idx < checkboxes.length; idx++){
            ...
            if (checkbox instanceof HTMLInputElement) values.push(checkbox.value); // add all values of checkboxes to array
        }
...
/* A web component to display activities */ class ResultDisplay extends HTMLElement {
...
class View {
    constructor(){
        ...
        if (!(this.results instanceof ResultDisplay)) console.log("No result-display element");
        this.clearButton = document.querySelector("#clear-button");
        if (!(this.clearButton instanceof HTMLButtonElement)) console.log("No clear button");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["d8lhj","jeorp"], "jeorp", "parcelRequire5af9")
